...
public:
    vector<vector<int> > Print(TreeNode* pRoot) {
        ...
        while(!level[0].empty()||!level[1].empty())//2：循环条件是两个栈有一个不空即可~  
            ...
            {
            TreeNode* p=level[current].top();//2.1：注意取的都是current层的node，存的都是next层的node  
            ...
            if(level[current].empty())//当前层已经被pop空？那就来清理一下现场进入下一轮的循环  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzYzNzUzMDM2LC0xOTQwOTI1NjQ3LDI5Mz
cyNDM3Myw5OTczODc2NTAsLTE1NDA3MjM2MjZdfQ==
-->